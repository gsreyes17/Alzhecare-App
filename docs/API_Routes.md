...
## Cobertura actual en la app Flutter (13-04-2026)

### Endpoints ya integrados y funcionales en UI

- `POST /api/auth/register`
  - Registro de paciente desde la pantalla inicial.
- `POST /api/auth/login`
  - Inicio de sesión para paciente, doctor y admin.
- `GET /api/auth/me`
  - Recuperación de perfil para bootstrap de sesión y refresco de datos.
- `PATCH /api/auth/me`
  - Actualización de nombre, apellido, correo y contraseña.
- `POST /api/auth/me/photo`
  - Actualización de foto de perfil.
- `POST /api/diagnoses/analyze`
  - Análisis de imagen propia.
- `GET /api/diagnoses/history?limit=50`
  - Historial propio con detalle e imagen analizada.
- `POST /api/doctor/patients/{patient_user_id}/analyze`
  - Análisis de imagen a nombre de paciente (flujo doctor/admin en UI).
- `GET /api/doctor/patients/{patient_user_id}/history?limit=50`
  - Historial de paciente por ID para revisión clínica.
- `GET /api/doctor/patients/search?q=ana`
  - Búsqueda incremental de pacientes por nombre, apellido o usuario.
- `POST /api/doctor/requests`
  - Envío de solicitud de vinculación por parte del doctor.
- `GET /api/doctor/requests`
  - Carga de solicitudes enviadas para marcar pendientes en la búsqueda.
- `GET /api/doctor/patients`
  - Lista de pacientes vinculados para análisis y creación de citas.
- `GET /api/patient/requests`
  - El paciente visualiza solicitudes pendientes.
- `PATCH /api/patient/requests/{request_id}`
  - El paciente acepta o deniega la solicitud.
- `GET /api/patient/notifications?unread_only=true`
  - El paciente consulta notificaciones (todas o solo no leídas).
- `PATCH /api/patient/notifications/{notification_id}/read`
  - El paciente marca notificaciones como leídas.
- `POST /api/doctor/appointments`
  - El doctor crea cita (dispara notificación al paciente).
- `GET /api/doctor/appointments?status=programada`
  - El doctor lista sus citas por estado.
- `PATCH /api/doctor/appointments/{appointment_id}/status`
  - El doctor actualiza estado de cita.
- `GET /api/admin/citas?status=programada`
  - El admin visualiza citas globales por estado.
- `PATCH /api/admin/citas/{appointment_id}/status`
  - El admin actualiza estado de cita global.

### Endpoints visibles en app pero pendientes de integración completa
